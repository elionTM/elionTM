...
const TechDevelopment = () => {
  ...
  const getIcon = (name) => {
    if (name.toLowerCase().includes('development') || name.toLowerCase().includes('stack')) return <Code className="w-6 h-6 text-purple-600" />;
    if (name.toLowerCase().includes('cloud') || name.toLowerCase().includes('infrastructure')) return <Database className="w-6 h-6 text-purple-600" />;
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigateTo('/')}>
            ...
            <span className="font-bold text-lg tracking-tight text-slate-900">Tech & Development</span>
          </div>
        </div>
      </nav>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center mb-20">
            ...
            >
              ...
              <h1 className="text-5xl font-bold text-slate-900 leading-tight">
                Building the <span className="text-purple-600">future</span> with modern tech.
              </h1>
              <p className="text-lg text-slate-600 leading-relaxed">
                We specialize in creating high-performance software solutions that are scalable, secure, and built to last.
              </p>
            </motion.div>
              ...
              services.map((service, index) => (
                ...
                >
                  ...
                  <button 
                    ...
                    className="w-full py-3 bg-purple-50 text-purple-600 rounded-2xl font-bold text-sm hover:bg-purple-600 hover:text-white transition-all flex items-center justify-center gap-2 group"
                  >
